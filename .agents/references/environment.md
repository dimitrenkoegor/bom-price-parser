...
- `C:\Users\Huawei\Desktop\Project\bom-price-parser` — второй чекаут того же репозитория.
  Его перестройка на пакет `src/bomprice` (общее ядро с `count pars`, сентябрь 2026) в
  master не принята и осталась только в истории git. Живой — этот, монолитный
  `price_parser.py`.
