...
param(
    ...
    [ValidateSet('run', 'check', 'once', 'preview', 'autostart-install', 'autostart-remove')]
    [string]$Mode
)
...
function Test-Python {
    ...
    if (-not $version) {
        ...
        Say
        Say 'Установите Python 3.11 или новее:'
    ...
    if ([int]$parts[0] -lt 3 -or ([int]$parts[0] -eq 3 -and [int]$parts[1] -lt 11)) {
        Say "[!] Нужен Python 3.11 или новее. Установлен: $version"
        return $false
    }
