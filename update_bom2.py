"""
Обновляет final/BOM_Запрос_2.xlsx найденными ценами (новый запрос).
Запускать: python update_bom2.py
"""
...
OUTPUT = SCRIPT_DIR / "final" / "BOM_Запрос_2.xlsx"
...
# ── Данные (собраны с oemsecrets/octopart/Mouser, 2026-06-26) ─────────────────
# EUR-цены конвертируются в USD по rate_eur при запуске
# price_eur OR price_usd (если USD уже известен)

RESULTS = [
    # 1 – Panasonic EXB2HV330JV, qty 5800
    # Avnet America: 65k в наличии, MOQ 5000, цена при 5000+: €0.0318
    {"pn": "EXB2HV330JV", "qty": 5800, "manufacturer": "Panasonic",
     "distributor": "Avnet", "moq": 5000, "stock": 65000, "lead": None,
     "price_eur": 0.0318, "status": "FOUND"},

    # 2 – Raltron CL7050-50.000-3.3-25-X-T-TR, qty 3500
    # Нет ни у одного авторизованного дистрибьютора
    {"pn": "CL7050-50.000-3.3-25-X-T-TR", "qty": 3500,
     "manufacturer": "Raltron Electronics", "status": "RFQ"},

    # 3 – Skyworks 511BBA50M0000AAGR, qty 3500
    # Mouser: 0 в наличии, 8 недель, MOQ 1000, цена при 1000: €4.03
    {"pn": "511BBA50M0000AAGR", "qty": 3500, "manufacturer": "Skyworks Solutions",
     "distributor": "Mouser", "moq": 1000, "stock": 0, "lead": 8,
     "price_eur": 4.03, "status": "FOUND"},

    # 4 – Renesas XLL736050.000000I, qty 3500
    # PN в запросе: XLL736050.0000001 (последний символ — буква I, не цифра 1)
    # Mouser: 423 в наличии, MOQ 1, цена при 1000+: $2.96
    {"pn": "XLL736050.000000I", "qty": 3500, "manufacturer": "Renesas Electronics",
     "distributor": "Mouser", "moq": 1, "stock": 423, "lead": None,
     "price_usd": 2.96, "status": "FOUND"},

    # 5 – EPSON SG-8002CA-PCM 50.0 MHz, qty 1150
    # DigiKey: 99 недель, price on request; только неавторизованные дистрибьюторы
    {"pn": "SG-8002CA-PCM 50.0 MHz", "qty": 1150,
     "manufacturer": "Epson", "status": "RFQ"},
]
...
def build_bom(rate_eur, rate_rub):
    ...
    note = (
        ...
        f"Авторизованные: Avnet, Mouser. "
        f"Позиция 4 (XLL736050.000000I): в запросе PN оканчивается на '1', "
        f"корректный символ — латинская 'I'. "
        f"Позиция 2 (CL7050): нет у авторизованных дистрибьюторов. "
        f"Позиция 5 (SG-8002CA-PCM): снято с производства, "
        f"DigiKey — 99 недель/price on request."
    )
